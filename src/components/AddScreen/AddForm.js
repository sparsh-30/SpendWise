import {View, TouchableNativeFeedback, Text, TextInput, Image} from 'react-native';
...
import { getExpenseCategoryArray, getIncomeCategoryArray } from '../../categoryLinks';
...
export default function AddForm() {
  ...
  return (
    <View className="flex-1 mt-4">
      <Text style={{color:theme==='light'?colors.light.primary:colors.dark.primary}} className="text-2xl text-center mb-10 font-[700]">ADD TRANSACTION</Text>
      <Formik
        initialValues={{email: ''}}
        ...
        {({handleChange, handleBlur, handleSubmit, values}) => {
           ...
           <View className="px-7">
            ...
            <TextInput
              ...
              // value={values.email}
              style={{color: theme==='light'?colors.light.text:colors.dark.text,borderColor:theme==='light'?colors.light.primary:colors.dark.primary}}
              ...
              selectionColor={theme==='light'?colors.light.text:colors.dark.text}
            />
            ...
            <View className="flex flex-row mb-6">
              ...
              <View className="w-1/2 relative bottom-[1]">
                <TransactionTypeDropdown getSelectedTransactionType={getSelectedTransactionType} />
              </View>
            </View>
            {/* Transaction Category Dropdown */}
            <View className="mb-6">
              <CategoriesDropdown selectedTransactionType={selectedTransactionType} />
            </View>
            ...
            <View>
              ...
              <View className="flex flex-row justify-between">
                <TouchableNativeFeedback onPress={showDatepicker}>
                  <View style={{backgroundColor:theme==='light'?colors.light.primary:colors.dark.primary}} className="w-[30%] py-3 flex flex-row justify-center rounded-md">
                    <Text className="text-[16] text-white font-[800]">DATE</Text>
                  </View>
                </TouchableNativeFeedback>
                <TouchableNativeFeedback onPress={showTimepicker}>
                  <View style={{backgroundColor:theme==='light'?colors.light.primary:colors.dark.primary}} className="w-[30%] py-3 flex flex-row justify-center rounded-md">
                    <Text className="text-[16] text-white font-[800]">TIME</Text>
                  </View>
                </TouchableNativeFeedback>
                <TouchableNativeFeedback onPress={handleClickOnNow}>
                  <View style={{backgroundColor:theme==='light'?colors.light.primary:colors.dark.primary}} className="w-[30%] py-3 flex flex-row justify-center rounded-md">
                    <Text className="text-[16] text-white font-[800]">NOW</Text>
                  </View>
                </TouchableNativeFeedback>
                {show && (
                  <RNDateTimePicker
            ...
            {/* Submit Button */}
            <TouchableNativeFeedback>
              <View style={{backgroundColor:theme==='light'?colors.light.primary:colors.dark.primary}} className="py-3 mt-10 flex flex-row justify-center rounded-md">
                <Text className="text-[16] text-white font-[800]">SUBMIT</Text>
              </View>
            </TouchableNativeFeedback>
          </View>
        )}}
...
const CategoriesDropdown = ({selectedTransactionType}) => {
  ...
  const categoriesArray=selectedTransactionType==='expense'?getExpenseCategoryArray():getIncomeCategoryArray();

  let items = categoriesArray.map(category => {
    return {
  ...
  return (
    <DropDownPicker
      ...
      ArrowUpIconComponent={() => (
        <Icon name="caretup" size={18} color={theme==='light'?colors.light.primary:colors.dark.primary} />
      )}
      ArrowDownIconComponent={() => (
        <Icon name="caretdown" size={18} color={theme==='light'?colors.light.primary:colors.dark.primary} />
      )}
      TickIconComponent={() => (
        <Check name="checkmark-circle-outline" size={24} color={theme==='light'?colors.light.primary:colors.dark.primary} />
      )}
      dropDownContainerStyle={{
        borderColor:
          theme === 'light' ? colors.light.primary : colors.dark.primary,
